...
let content = fs.readFileSync('old_chat.tsx', 'utf16le');
if (content.indexOf('PageShell') === -1) {
  content = fs.readFileSync('old_chat.tsx', 'utf8');
}
...
// We need to find `</PageShell>` to know where the return ends!
const returnEndIdx = content.indexOf('</PageShell>', returnStart);

if (returnEndIdx === -1) {
    console.log("Could not find end of return");
    process.exit(1);
}

const endOfFunc = content.indexOf('}', returnEndIdx);

const afterReturn = content.substring(endOfFunc + 1);
